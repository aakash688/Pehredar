...
if ($ticketId <= 0) { 
    send_error_response('ticket_id required', 400); 
}

// Ensure ticket belongs to the user's society
...
$societyId = (int)$t['society_id'];
if ($societyId != $user_data->society->id) { 
    send_error_response('You do not have permission to upload to this ticket', 403); 
}
...
function save_one_file($file, $ticketId, $userId, $pdo, $commentId = 0) {
    ...
    if ($destAbs === false) { 
        @mkdir(__DIR__ . '/../../uploads/tickets', 0775, true); 
        $destAbs = realpath(__DIR__ . '/../../uploads/tickets'); 
    }
...
if (!empty($_FILES['attachments']) && is_array($_FILES['attachments']['name'])) {
    ...
    for ($i = 0; $i < $count; $i++) {
        ...
        $one = save_one_file($file, $ticketId, $user_data->id, $pdo, $commentId);
        if ($one) { 
            $saved[] = $one; 
        }
    }
    ...
    $one = save_one_file($_FILES['attachment'], $ticketId, $user_data->id, $pdo, $commentId);
    if ($one) { 
        $saved[] = $one; 
    }
} elseif (!empty($_FILES['photo'])) {
    $one = save_one_file($_FILES['photo'], $ticketId, $user_data->id, $pdo, $commentId);
    if ($one) { 
        $saved[] = $one; 
    }
} elseif (!empty($_FILES['file'])) {
    $one = save_one_file($_FILES['file'], $ticketId, $user_data->id, $pdo, $commentId);
    if ($one) { 
        $saved[] = $one; 
    }
}
...
if (empty($saved)) { 
    send_error_response('No files uploaded', 400); 
}

send_json_response(['success' => true, 'attachments' => $saved]);
